...
function zip_logical_links() {
    ...
    candidate_dirs="lib /home/loreto/GIT-REPO/ESP32/esp32LnLibrary/lnLibrary"

    zipName="${PWD}/Source/lnLibary.zip"
    ...
    for _dir in $candidate_dirs; do
        if [ -d "$_dir" ]; then
            ...
            for name in *; do
                ...
                zip -x '*.git*' -x '*.pio*' -ur --latest-time  $zipName $name
            done
        fi
