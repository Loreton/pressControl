...
#pragma once
    ...
    // #endif
        ...
        public:
            ...
            inline bool onXSeconds(uint32_t modulo, bool trueOnFirstRun) {onSecondModulo(modulo, trueOnFirstRun); }
    ...
    // #ifdef ON_TIME_CLASS_MINUTES_VECTOR
            // --- per i vector
            std::vector<uint8_t> m_at_minutes_to_monitor; // array dei minute da controllare
            std::map<uint8_t, bool> m_at_minute_flags;    // map contenente lo stato dei minutei
            std::map<uint8_t, int8_t> m_at_last_minute_vector_map; // per atMinuteFlag()
    ...
    // #endif
        ...
        public:
            ...
            bool onMinuteModulo(uint32_t modulo, bool trueOnFirstRun=false); // on minute xx modulo  (ex.: atMinuteModulo(20) return true at minute 20, 40, 0)
            inline bool onXMinutes(uint32_t modulo, bool trueOnFirstRun) {onMinuteModulo(modulo, trueOnFirstRun); }
